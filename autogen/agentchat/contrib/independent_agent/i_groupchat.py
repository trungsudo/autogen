...
class AgentWrapper(Agent):
    ...
    def generate_reply(self, sender: Agent):
        ...
        print(
            colored(f"{sender._name} is requesting {self._name} to reply", "blue"),
            flush=True,
        )
...
class IndependentGroupChat(GroupChat):
    def __init__(self, agents: Union[List[Agent], List[str]], sa_agent_config: Optional[Dict] = None, *args, **kwargs):
        ...
        _agents = [
            ...
            else AgentWrapper(self._central_server, get_agent_online(self._central_server, agent))
            for agent in agents
        ]
        super().__init__(agents=_agents, *args, **kwargs)


class IndependentGroupChatManager(IndependentAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.register_reply(Agent, IndependentGroupChatManager.run_chat, config=groupchat, reset_config=GroupChat.reset)
        # Allow async chat if initiated using a_initiate_chat
        self.register_reply(
            Agent, IndependentGroupChatManager.a_run_chat, config=groupchat, reset_config=GroupChat.reset
        )
    ...
    def run_chat(
        ...
        for i in range(groupchat.max_round):
            ...
            try:
                ...
                print(colored(f">>>> NEXT SPEAKER: {speaker._name}", "light_cyan"), flush=True)
            ...
            self._process_received_message(str(reply), speaker, False)
...
def get_agent_online(central_server, agent_name: str) -> AgentInfo:
    info = get_agentinfo_from_name(central_server=central_server, agent_name=agent_name)
    print(
        colored(f"Member {agent_name} is {info.status}", "green" if info.status == RunningState.ONLINE else "red"),
        flush=True,
    )
